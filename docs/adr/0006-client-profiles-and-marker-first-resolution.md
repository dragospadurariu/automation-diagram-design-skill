...
Named profiles are full style-guide snapshots in `~/.automation-design/profiles/`, with one metadata header naming the active profile. An optional project-root `.automation-design` marker selects a validated slug and reads that home profile directly; only an explicit markerless load copies a profile into the installed working file. Loads check the current semantic-role and typography schema and backfill missing rows from shipped defaults.
