...
**2026-08-20 — the pattern count is 20; the type count is still 11.** **Application card** was added, routing to the existing **Architecture** type (screen navigation routes to the existing State machine via a flavor; the runtime-topology annex is plain Architecture with layer zones): an application documented as its functional-context artifact, with a conditional screen-contract drill-down. No new layout grammar — the counter moved 19 → 20 in the same commit as this amendment, per the procedure above.
