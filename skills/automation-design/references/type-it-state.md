...
**Best for:** documenting the *before* picture of a modernization proposal — the legacy IT landscape grouped by phase or department (Collection → Processing → Dissemination, or Frontend / Backend / Storage, or Survey → Analysts → Reports), with pain-points flagged, file-based hand-offs labelled (CSV / Excel / Email / Copy), and pre-platform tooling visible. The companion to `type-architecture.md`: this type shows the gap that a target-topology proposal is going to close.
...
This type is **parametric** — the inputs schema in §1 drives every coordinate via the formulas in §2. The zone + cross-cutting footer-bar frame is unique to this type, but the rule shape mirrors `type-process.md` (rounded right-angle connectors) and `type-data-flow.md` (per-element `color` override) so the focal rule, color override, dark mode, and reproducibility checklist read identically across types.
...
**Recommended cross-type palette** (shares its base hues with `type-process.md` / `type-data-flow.md` — don't diverge):
...
- **Footer bar wired to one component.** Footer = cross-cutting layer-wide concern; a connector from a footer to a specific component is a category error (the one exception is an AUTH line drawn when the footer service truly authenticates *all* components, and even then the line lands at the zone bottom edge, not at a specific tool).
