...
- Dot pattern: 22×22 grid, `circle r=0.8`, `fill rgba(30,30,30,0.10)`. Opacity 0.55.
- Alternating lane tints: odd-indexed lanes (0, 2, …) receive `rgba(30,30,30,0.018)` fill from `x=140` to `viewBox_w`.
- Lane dividers: horizontal hairlines at every `lane_y_top(k)` and at `legend_y_top`; stroke `rgba(30,30,30,0.12)` width 0.8.
- Label column right border: vertical hairline at `x = label_col_w`, stroke `rgba(30,30,30,0.20)` width 1, from `y = header_h` to `y = legend_y_top`.
...
- Default fill: `rgba(30,30,30,0.12)`, number text ink.
- Focal fill: `rgba(8,122,91,0.20)`, number text accent (§5).
...
- Default fill: muted (`#4a5464` light / `#c1c8d1` dark).
- Focal fill: accent (`#087a5b` light / `#82ffcf` dark).
...
|---|---|---|---|---|---|
| `normal` | `#4a5464` (muted) | 1.0 | — | `arrow` | Standard data hand-off between steps or actors. Unlabelled. |
| `focal-in` / `focal-out` | `#087a5b` (accent) | 1.2 | — | `arrow-accent` | Every edge whose endpoint is the focal node (`focal-in`) or origin is the focal node (`focal-out`). |
| `trigger` | `#4a5464` (muted) | 1.0 | `4,3` | `arrow-sm` | Orchestration trigger (scheduler → tool, manual override → upstream step). Unlabelled. |
...
<defs>
  <pattern id="dots" width="22" height="22" patternUnits="userSpaceOnUse">
    <circle cx="11" cy="11" r="0.8" fill="rgba(30,30,30,0.10)"/>
  </pattern>
  <marker id="arrow"        markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#4a5464"/></marker>
  <marker id="arrow-accent" markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#087a5b"/></marker>
  <marker id="arrow-sm"     markerWidth="6" markerHeight="5" refX="5" refY="2.5" orient="auto"><polygon points="0 0, 6 2.5, 0 5" fill="#4a5464"/></marker>
</defs>
```
...
Any node, lane, or step accepts an optional `color: "#hex"`. Mirrors `type-data-flow.md` §4 and `type-it-state.md` §4 so the rule reads identically across types.
...
Same palette as `type-data-flow.md`, `type-it-state.md`:
...
| Muted | `#4a5464` | `#c1c8d1` |
| Soft | `#7b8797` | `#9aa4b1` |
| Accent | `#087a5b` | `#82ffcf` |
| Dot pattern | `rgba(30,30,30,0.10)` | `rgba(246,246,246,0.10)` |
| Lane tint | `rgba(30,30,30,0.018)` | `rgba(246,246,246,0.025)` |
| Dividers | `rgba(30,30,30,0.12)` | `rgba(246,246,246,0.12)` |
| Label col divider | `rgba(30,30,30,0.20)` | `rgba(246,246,246,0.22)` |
| Default chip fill | `rgba(30,30,30,0.12)` | `rgba(246,246,246,0.12)` |
| Focal chip fill | `rgba(8,122,91,0.20)` | `rgba(130,255,207,0.22)` |
| Default node fill | white | `rgba(246,246,246,0.04)` |
| Default node stroke | `rgba(30,30,30,0.25)` | `rgba(246,246,246,0.20)` |
| Focal node fill | `rgba(8,122,91,0.08)` | `rgba(130,255,207,0.12)` |
| Focal node stroke | `#087a5b` | `#82ffcf` |
