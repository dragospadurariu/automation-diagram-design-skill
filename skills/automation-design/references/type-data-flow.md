...
node_w           = 100
node_h           = 64
fan_offset       = 24                                                 # second connector on an occupied edge (§3.1)
...
- **Same-step cross-lane, fanned**: when the centreline point on the edge a new same-step connector would use is already occupied — typically by the incoming focal arrow — offset the new one by `fan_offset` (§2) and run it as a straight vertical from the source's bottom edge `(step_cx(j) + fan_offset, node_y(k_from) + node_h)` to the target's top edge `(step_cx(j) + fan_offset, node_y(k_to))`, crossing any intervening lane. `fan_offset = 24` satisfies both the 4-px grid and SKILL.md §6 rule 4's ≥12 px attach-point separation. Used when two continuations leave one decision node — a human approval whose approve path exits right and whose reject path drops to an exception store in another lane.
...
Any node, lane, or step may declare an optional `color: "#hex"`. The rule is written to read identically wherever a type offers per-component color, so keep this wording if another type adopts it.
...
One shared palette, so a reader scanning several diagrams sees the same colors meaning the same thing:
