...
| Wall-print process map, PDD annex | `png` @3 | `print-a3-landscape` |
...
| `print-a3-landscape` | `0 0 1584 1120` | ~1.41:1 | @3 → 4752×3360 | print | A3 landscape; the large-canvas preset for phased process maps and detail-set overviews |
...
`print-a3-landscape` keeps the **print** ramp unchanged: the extra ~2× area buys *nodes and phases*, never smaller type or tighter gaps. It exists for detailed processes — a phased blueprint gains sanctioned budget headroom on it (the extended budget lives in [type-flowchart.md § Phased blueprint variant](type-flowchart.md), which owns the numbers), and a `faithful` import may use the full 24-node ceiling with room to route cleanly. If even A3 feels tight, the answer is a **detail set** (overview + one numbered page per process — [type-flowchart.md § Detail set](type-flowchart.md)), not a bigger canvas.
