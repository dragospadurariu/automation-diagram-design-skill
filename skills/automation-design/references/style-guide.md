...
A small set of desaturated, editorial-tone colors for chart types that genuinely need to distinguish multiple overlapping entities. No type this skill currently ships uses it; the palette stays defined here so that any type which later needs multiple series draws from it instead of inventing its own colors. The "1-focal" rule still holds — `accent` is reserved for the focal series; the palette below covers the rest.
