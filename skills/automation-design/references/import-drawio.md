...
Load this file when the user points at a `.drawio`, `.drawio.xml`, `.drawio.png`, or `.drawio.svg` file and wants a diagram out of it — "convert this drawio", "redraw this diagram", "make this presentable", "この drawio をきれいにして", or the `/automation-design:import-drawio` slash command.
...
`<skill-dir>` is `skills/automation-design/` in this repo, or the skill's own directory when it's installed standalone or as a plugin. If the path isn't obvious, glob for `**/automation-design/scripts/drawio_extract.py`.
...
| Nested containers, depth ≥2, few edges | Architecture, drawn as grouped zones | [type-architecture.md](type-architecture.md) |
| One entry point, no cycle, fan-out only, **nodes are people / teams / agents / owners** | Org chart | [type-org-chart.md](type-org-chart.md) |
| Same shape but the nodes are *things* — modules, taxonomies, folders | Architecture with grouped zones if the grouping carries the meaning; otherwise **out of scope** | [type-architecture.md](type-architecture.md) — or see *No surviving target type* below |
