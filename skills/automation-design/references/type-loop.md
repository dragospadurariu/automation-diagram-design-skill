...
Loop's circular ring arcs are a documented type-specific exception to SKILL.md §6 rule 1 — the only connector-shape exception this skill grants any type. A Loop never mixes cubic, straight, or rounded-orthogonal segments into its ring: the six visible gaps must read as pieces of one continuous circle.
...
SKILL.md §6 applies in full except for the two Loop-specific connector primitives: circular ring arcs (§2.2) and straight radial spokes (§2.3). These two, and only these, replace §6 rule 1 for this diagram type:
