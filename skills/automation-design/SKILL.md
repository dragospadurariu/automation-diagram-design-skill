...
| Dispatcher enqueues work items, performers consume transactions | **Dispatcher / queue / performer** → Data flow |
| Robot and human share a desktop, taking turns on one workstation | **Attended automation handoff** → Swimlane |
| Work-item states with retries, business vs. system exceptions | **Transaction lifecycle with retry and exceptions** → State machine |
| Automation suspends until a person validates, approves, or rejects | **Human-in-the-loop approval** → Sequence |
| Documents classified, extracted, validated, posted downstream | **Document processing pipeline** → Data flow |
| LLM agent reasons over a goal and calls tools, MCP servers, or APIs | **Agent with tools** → Architecture |
| Agent decides; deterministic RPA, API, or workflow executes | **Agent → RPA handoff** → Sequence |
| Supervisor routes work to specialist worker agents and owns escalation | **Supervisor and worker agents** → Org chart |
| Agent acts, observes results, updates memory, improves | **Agent memory and evaluation loop** → Loop |
...
| States + transitions + guards — transaction and work-item lifecycles | **State machine** | [type-state.md](references/type-state.md) |
| Cross-functional process with handoffs between humans, robots, and agents | **Swimlane** | [type-swimlane.md](references/type-swimlane.md) |
...
- Automation vocabulary, badges, and boundaries → [automation-primitives.md](references/automation-primitives.md)
...
2. **Label-to-connector margin: 6–10px gap, always.** A label must never sit *on* its arrow — the connector must remain visible. Place the label centered above (or beside, for vertical segments) the line with a **minimum 6px gap** between the bottom of the label's mask rect and the connector stroke. The opaque mask rect prevents the arrow from bleeding through, but the *visible* gap between mask edge and line preserves the reader's ability to trace the connection. If 6px feels cramped, push to 8–10px. Never let the mask touch or overlap the stroke.

3. **No overlapping connectors.** Two connectors must never share the same stroke path, run parallel on top of each other, or be drawn on top of each other for any segment. When two orthogonal arrows must cross at a single point, apply the **bridge / hop** primitive (see `references/type-architecture.md` § Crossing arrows). When two arrows naturally want to overlap, offset their routing by ≥12px so each line is independently traceable. If you're stacking connectors, redesign the layout — the nodes are too close or the diagram is over budget (split it).
...
7. **Every arrowhead lands on a border** — a node or gate rect, a circle/polygon outline, or a lifeline segment; never short (floating) or past it (buried). Decision diamonds are `<polygon>`, not `<path>`. `python3 scripts/verify-geometry.py` enforces rules 2, 3, 6, 7 and §5 burial.
...
If you exceed, split into two diagrams (overview + detail). For a detailed end-to-end process, the split is a formalized deliverable — the **detail set** (numbered overview + one process-detail page per phase, [type-flowchart.md § Detail set](references/type-flowchart.md)) — and the `print-a3-landscape` preset gives the overview sanctioned budget headroom ([output-spec.md §2](references/output-spec.md)).
...
- `faithful` is one of the two documented exemptions from the §7 complexity budget, and it's conditional: above 9 nodes the layout must be zoned, above 24 it must split into overview + detail. (The other is the **RPA solution blueprint** pattern, whose phase banding and numbered single process are sanctioned zoning — see [type-flowchart.md](references/type-flowchart.md).) The connector rules in §6 never relax.
